...
// Функция для обработки стилей
function styles() {
    return src('app/scss/style.scss') // Указываем исходный SCSS файл
        .pipe(newer('app/css/style.min.css')) // Проверяем, изменился ли файл
        .pipe(postcss([autoprefixer({ overrideBrowserslist: ['last 10 versions'] })])) // Добавляем префиксы
        .pipe(concat('style.min.css')) // Объединяем в один файл
        .pipe(scss({ style: 'compressed' }).on('error', function (err) {
...
// Функция для наблюдения за изменениями файлов
function watching() {
    watch(['app/scss/style.scss'], { usePolling: true }, styles).on('change', () => {
        console.log('SCSS файл изменён');
    }); // Следим за SCSS файлами
    watch(['app/js/main.js', '!app/js/main.min.js'], { usePolling: true }, scripts).on('change', () => {
